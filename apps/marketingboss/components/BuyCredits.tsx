...
export default function BuyCredits({
  ...
  return (
    <div className="flex flex-col gap-4">
      {!configured && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3.5 text-sm text-amber-700">
          Card payments aren&apos;t switched on yet. Buttons will work once billing is configured.
        </div>
      )}
      {error && (
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-3.5 text-sm text-red-600">{error}</div>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {packs.map((pack) => (
          ...
          >
            ...
            <p className="mt-2 min-h-10 text-xs leading-relaxed text-slate-500">{pack.blurb}</p>
            <button
              onClick={() => buy(pack.id)}
