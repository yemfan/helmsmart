 ...
 * looks wrong in source. Ported from
 * `apps/leadsmartai/lib/i18n/__tests__/localeBlindDates.test.ts`, where it
 * slipped through four separate times in three different disguises:
 *
 ...
 * the argument silently stays English while looking correct. HelmSmart's
 * `docs/i18n-howto.md` names that last shape explicitly: "a helper with a
 * default of `en-US` is a bug the guard catches".
 *
 * So: if a file is internationalised, its dates must be too. Pass the locale
 * from `intlLocale(i18n.language)` (client) or
 * `intlLocale(await getServerLocale())` (server).
 */
 ...
 * file — which cost several minutes each time this fired.
 */
...
describe("locale-blind dates", () => {
  ...
  it("recognises all four disguises, and leaves a passed locale alone", () => {
    const hits = (line: string) => OFFENDERS.some((re) => re.test(line));
    expect(hits('d.toLocaleDateString("en-US", { month: "short" })')).toBe(true);
    expect(hits("d.toLocaleDateString(undefined)")).toBe(true);
    expect(hits("d.toLocaleDateString()")).toBe(true);
    expect(hits('function fmt(iso: string, locale = "en-US") {')).toBe(true);
    expect(hits("d.toLocaleDateString(intlLocale(locale), { month: 'short' })")).toBe(false);
    // Integers format identically in en-US and zh-CN, so a bare number is not
    // worth failing over — only Date-valued calls are.
    expect(hits("n.toLocaleString()")).toBe(false);
  });
});
