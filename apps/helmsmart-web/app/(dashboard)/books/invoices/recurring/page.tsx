...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t("invoices.recurring.metaTitle") };
}
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-5 mb-6 text-sm text-slate-500">
        <span className="flex items-center gap-2">
          ...
          {t("invoices.recurring.activeCount", { count: activeCount })}
        </span>
        {pausedCount > 0 && (
          <span className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-amber-400" />
            {t("invoices.recurring.pausedCount", { count: pausedCount })}
          </span>
        )}
        <span className="text-xs text-slate-400">
          {t("invoices.recurring.autoGeneratedHint")}
        </span>
      </div>
        ...
        {!recurring.length ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xs text-slate-400 max-w-xs mb-5">
              {t("invoices.recurring.emptyBody")}
            </p>
            <RecurringInvoiceModal
              currency={currency}
              clients={
          ...
          <>
            ...
            <div className="grid grid-cols-[1fr_120px_100px_120px_100px] gap-4 px-6 py-3 bg-slate-50 border-b border-slate-100 text-xs font-medium text-slate-500 uppercase tracking-wide">
              <span>{t("invoices.recurring.columns.client")}</span>
              <span>{t("invoices.recurring.columns.frequency")}</span>
              <span>{t("invoices.recurring.columns.amount")}</span>
              <span>{t("invoices.recurring.columns.nextInvoice")}</span>
              <span className="text-right">{t("invoices.recurring.columns.status")}</span>
            </div>
