...
/**
 * Standard on/off switch for all HelmSmart toggles.
 *
...
export function Toggle({
  ...
  const shift = size === "sm" ? (checked ? "translate-x-3.5" : "translate-x-0.5") : checked ? "translate-x-4" : "translate-x-0.5";
  return (
    <button
      type="button"
      role="switch"
    ...
    >
      <span className={`inline-block ${thumb} transform rounded-full bg-white shadow transition-transform ${shift}`} />
    </button>
  );
}
