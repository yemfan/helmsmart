...
export function AssistantCallSettings({
  ...
  useEffect(() => {
    let alive = true;
    fetch(`/api/dashboard/closeboss/assistant-voice?type=${type}`)
      .then((r) => r.json())
      .then((res) => {
        if (!alive) return;
  ...
  async function save(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/dashboard/closeboss/assistant-voice", {
        method: "PATCH",
  ...
  if (loading) return <p className="py-4 text-sm text-slate-500">{t("pages.assistantCallSettings.loadingCallSettings")}</p>;
...
export function ReceptionistVoiceForm() {
  ...
  if (loading) return <p className="py-4 text-sm text-slate-500">{t("pages.assistantCallSettings.loadingVoiceSettings")}</p>;
