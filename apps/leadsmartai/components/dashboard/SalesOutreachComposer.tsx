...
export default function SalesOutreachComposer({
  ...
  return (
    <section ref={rootRef} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-sm">
        ...
        <div ref={boxRef} className="relative mb-4">
          ...
          {open && reachable.length > 0 && (
            <div className="absolute z-10 mt-1 max-h-64 w-full overflow-auto rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 py-1 shadow-lg">
              {filtered.length === 0 ? (
                <div className="px-3 py-2 text-xs text-slate-500">{t("outreach.noMatching")}</div>
              ) : (
                filtered.map((c) => (
                  <button
                    key={c.id}
      ...
      />
      {channel === "sms" && (
        <p className="mb-4 text-[11px] text-slate-500">{t("pages.outreachComposer.stopNoticeAuto")}</p>
      )}
      ...
      {overCap && (
        <p className="mt-2 text-[11px] text-slate-500">{t("pages.dashFragments.batchesCappedAt")} {MAX_BULK} — this reaches the first {MAX_BULK} of {segmentCount}.
        </p>
      )}
      {feedback && (
    ...
    <button
      ...
      className={`flex flex-1 items-center justify-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition ${
        active
          ...
          : disabled
            ? "cursor-not-allowed border-slate-200 dark:border-slate-700 text-slate-500"
