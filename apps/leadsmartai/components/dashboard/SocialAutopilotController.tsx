...
export default function SocialAutopilotController() {
  ...
  if (!config) {
    return <p className="text-xs text-slate-500">{error ?? t("common:actions.loading")}</p>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {aiOn && (
        <p className="-mt-3 text-[11px] text-slate-500">{t("pages.socialAutopilot.onNotice")}</p>
      )}
      ...
      <p className="text-[11px] text-slate-500">{t("pages.socialAutopilot.approvalScope")}</p>
    </div>
  );
}
