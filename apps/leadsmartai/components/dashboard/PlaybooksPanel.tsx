  ...
  if (loading && !tasks.length) {
    return (
      <section className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-sm">
        <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">📋 Playbooks</h2>
      ...
      {batches.length === 0 ? (
        ...
        <div className="mt-3 space-y-4">
          {batches.map((batch) => (
            ...
            >
              <div className="flex items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/60 px-3 py-2">
...
export function PlaybookPickerModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-xl">
              ...
              playbooks.map((p) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-[11px] text-slate-600 dark:text-slate-400">
        ...
        <div className="mt-4 flex items-center justify-end gap-2">
          {stage === "review" ? (
            <button
              ...
              className="rounded-lg border border-slate-200 dark:border-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"
            >
              ← Back
            </button>
          ) : (
    ...
    <div className="mt-4 space-y-4">
      {contactPicker ? (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
          ...
          {contactPicker.leads.length === 0 ? (
            <div className="mt-2 rounded-md border border-amber-300 bg-white dark:bg-slate-900 px-3 py-2 text-xs text-amber-900">{t("pages.dashFragments.noContactsYet")}{" "}
              <a
            ...
            <select
              ...
              className="mt-2 w-full rounded-md border border-amber-300 bg-white dark:bg-slate-900 px-3 py-2 text-sm"
            >
      ...
      <div className="rounded-lg bg-slate-50 dark:bg-slate-900/60 p-3">
        ...
        <input
          ...
          className="mt-1 w-full rounded-md border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm"
        />
      </div>
      ...
      <div className="max-h-[360px] overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-700">
        {grouped.map(([section, items], gi) => (
          <div key={`${section}-${gi}`}>
            {section ? (
              <div className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/60 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                {section}
              </div>
            ) : null}
