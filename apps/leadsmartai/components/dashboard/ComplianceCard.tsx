...
export default async function ComplianceCard() {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="text-sm font-semibold text-slate-900">{t("pages.compliance.title", { ns: "dashboard" })}</h2>
        ...
        {ITEMS.map((item) => (
          <div key={item.label} className="p-3">
            <div className="flex items-start justify-between gap-3">
              <div className="text-sm font-medium text-slate-900">{t(item.label, { ns: "dashboard" })}</div>
            ...
            <div className="mt-1 text-xs text-slate-600">{t(item.description, { ns: "dashboard" })}</div>
            {item.source && (
      ...
      <p className="mt-4 text-[11px] text-slate-500">
        <strong className="font-semibold text-slate-700">{t("pages.compliance.legalNoticeLabel", { ns: "dashboard" })}</strong>{" "}
