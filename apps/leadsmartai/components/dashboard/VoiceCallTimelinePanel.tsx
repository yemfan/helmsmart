  ...
  return (
    <section className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
      <header className="mb-3">
        <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{t("pages.voiceCallTimeline.title")}</h2>
        <p className="mt-0.5 text-xs text-slate-600 dark:text-slate-400">{t("pages.voiceCallTimeline.sub")}</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/60 px-4 py-6 text-center text-xs text-slate-600 dark:text-slate-400">{t("pages.voiceCallTimeline.empty")}</div>
      ) : (
