  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(280px,340px)_1fr]">
      <aside className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
        <div className="space-y-2 border-b border-slate-100 dark:border-slate-700 p-3">
          <input
            type="search"
            placeholder={t("pages.templatePicker.searchByNameIdBody")}
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="block w-full rounded-lg border border-slate-300 dark:border-slate-700 px-3 py-2 text-sm"
          />
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          {(Object.keys(grouped) as TemplateCategory[]).map((cat) => {
            ...
            return (
              <div key={cat}>
                <div className="sticky top-0 bg-slate-50/90 dark:bg-slate-900/60 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500 backdrop-blur">
...
function TemplateDetail({
  ...
  return (
    <div className="flex h-full flex-col">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 text-[10px] text-slate-500">
            ...
            <h2 className="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-100">{template.name}</h2>
            {template.notes && (
      ...
      <div className="grid flex-1 grid-cols-1 divide-slate-100 dark:divide-slate-800 lg:grid-cols-2 lg:divide-x">
        <section className="flex flex-col gap-3 p-4">
          ...
          <label className="block flex-1">
            ...
            <textarea
              ...
              className="mt-1 block w-full rounded-lg border border-slate-300 dark:border-slate-700 px-3 py-2 text-sm font-mono leading-relaxed"
            />
          </label>
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap gap-1">
                {template.placeholders.map((p) => (
                  <span
                    key={p}
                    className="rounded bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 font-mono text-[10px] text-slate-600 dark:text-slate-400"
                  >
                    {`{{${p}}}`}
                  </span>
                ))}
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 dark:border-slate-700 pt-3">
            ...
            {template.override?.edited && (
              <button
                ...
                onClick={() => void revert()}
                disabled={saving}
                className="rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"
        ...
        <section className="flex flex-col p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="inline-flex rounded-md border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/60 p-0.5 text-xs">
              {(["rendered", "raw"] as const).map((m) => (
                <button
                  ...
                  className={`rounded px-2 py-0.5 ${
                    previewMode === m ? "bg-white dark:bg-slate-900 shadow-sm text-slate-900 dark:text-slate-100" : "text-slate-500"
                  }`}
                >
          ...
          <div className="mt-3 flex-1 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/60 p-4">
...
function PreviewPane({
  ...
  return (
    <div className={`space-y-2 ${mono ? "font-mono text-xs" : "text-sm"}`}>
      ...
      <pre
        className={`whitespace-pre-wrap ${mono ? "font-mono text-xs text-slate-700" : "font-sans text-slate-800 dark:text-slate-200 leading-relaxed"}`}
      >
        {body}
      </pre>
    </div>
      ...
      {(["autosend", "review", "off"] as const).map((s) => (
        <button
          ...
          className={`rounded px-2.5 py-1 font-medium ${
            value === s ? "bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 shadow-sm" : "text-slate-500 hover:text-slate-700"
          }`}
        >
          {s === "autosend"
