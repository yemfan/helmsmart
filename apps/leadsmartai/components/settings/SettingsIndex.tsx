...
export function SettingsIndex() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-5">
        <h1 className="text-xl font-semibold text-gray-900">{t("settings.index.title")}</h1>
        <p className="mt-0.5 text-sm text-gray-500">{t("settings.index.subtitle")}</p>
      </div>
      <input
        type="search"
        value={query}
        ...
        className="mb-4 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:border-[#0072ce] focus:outline-none focus:ring-2 focus:ring-[#0072ce]/20"
      />
      {visible.length === 0 ? (
          ...
          {visible.map((g) => {
            ...
            return (
              <li key={g.id}>
                <Link
                  href={g.href}
                  className="group flex h-full items-start gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition hover:-translate-y-px hover:border-gray-300 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0072ce]/40"
                >
                  ...
                  <span className="min-w-0">
                    <span className="block text-sm font-semibold text-gray-900 group-hover:text-[#0072ce]">{g.label}</span>
                    <span className="mt-0.5 block text-xs leading-relaxed text-gray-500">{g.description}</span>
                  </span>
                </Link>
              </li>
            );
