...
export default async function DataCenterPage() {
  ...
  return (
    <main className="bg-white text-slate-900">
      ...
      <section className="border-b border-slate-100 bg-gradient-to-b from-slate-50 to-white">
        <div className="mx-auto max-w-7xl px-6 py-16 md:py-20">
          <nav className="mb-6 text-sm">
            ...
            <span className="text-slate-600">{t("pages.articleChrome.dataCenter", { ns: "web_pages" })}</span>
          </nav>

          <div className="max-w-3xl">
            <p className="mb-4 inline-flex rounded-full border border-slate-200/90 bg-white px-3 py-1 text-xs font-medium uppercase tracking-[0.15em] text-slate-600 shadow-sm ring-1 ring-slate-900/[0.03]">{t("pages.articleChrome.dataCenter", { ns: "web_pages" })}</p>
            <h1 className="text-4xl font-bold leading-tight tracking-tight md:text-5xl">{t("pages.dataCenterPages.heroTitle", { ns: "web_pages" })}</h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">{t("pages.dataCenterPages.heroBody", { ns: "web_pages" })}</p>
          </div>
        </div>
      </section>

      {/* Reports list */}
      <section className="mx-auto max-w-7xl px-6 py-14 md:py-16" aria-label={t("pages.dataCenterPages.researchAria", { ns: "web_pages" })}>
        <div className="mb-6 flex items-baseline justify-between gap-4">
          <h2 className="text-2xl font-semibold tracking-tight md:text-3xl">{t("pages.dataCenterPages.researchTitle", { ns: "web_pages" })}</h2>
          <span className="text-sm text-slate-500">{t("pages.dataCenterPages.newestFirst", { ns: "web_pages" })}</span>
        </div>

        {reports.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center">
            <p className="text-slate-600">{t("pages.dataCenterPages.reportsComing", { ns: "web_pages" })}</p>
          </div>
        ) : (
      ...
      <section className="bg-slate-50/80" aria-label={t("pages.dataCenterPages.exploreAria", { ns: "web_pages" })}>
        <div className="mx-auto max-w-7xl px-6 py-14 md:py-16">
          ...
          >
            <p className="inline-flex rounded-full bg-[#0072ce]/10 px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-[#0072ce]">{t("pages.dataCenterPages.localData", { ns: "web_pages" })}</p>
            <h2 className="mt-3 text-lg font-semibold tracking-tight text-slate-900">{t("pages.dataCenterPages.exploreTitle", { ns: "web_pages" })}</h2>
            <p className="mt-2 text-sm leading-relaxed text-slate-600">{t("pages.dataCenterPages.exploreBody", { ns: "web_pages" })}</p>
            <span className="mt-4 inline-flex items-center text-sm font-semibold text-[#0072ce] group-hover:text-[#005ca8]">
              {t("pages.dataPage.browseMarkets", { ns: "web_pages" })}
            </span>
          </Link>
        </div>
      </section>
