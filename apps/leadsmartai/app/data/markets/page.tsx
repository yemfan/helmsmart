    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="mx-auto max-w-5xl px-4 py-12 space-y-12">
        <nav className="text-sm">
          ...
          <span className="text-slate-400 mx-2">/</span>
          <span className="text-slate-600">{t("pages.dataCenterPages.markets", { ns: "web_pages" })}</span>
        </nav>

        <header className="space-y-4">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#0072ce]">{t("pages.dataCenterPages.marketsTitle", { ns: "web_pages" })}</p>
          <h1 className="text-4xl font-bold leading-tight text-slate-900">{t("pages.dataCenterPages.marketsH1", { ns: "web_pages" })}</h1>
          <p className="max-w-2xl text-lg leading-relaxed text-slate-600">{t("pages.dataCenterPages.marketsBody", { ns: "web_pages" })}</p>
        </header>

        <section aria-label={t("pages.dataCenterPages.nationalAria", { ns: "web_pages" })} className="space-y-4">
          <div className="flex items-baseline justify-between gap-4">
            <h2 className="text-2xl font-bold text-slate-900">{t("pages.dataCenterPages.nationalTitle", { ns: "web_pages" })}</h2>
            {periodLabel && (
              <span className="text-sm text-slate-500">As of {periodLabel}</span>
            )}
          </div>
          <p className="max-w-2xl text-sm leading-relaxed text-slate-600">{t("pages.dataCenterPages.nationalBody", { ns: "web_pages" })}</p>
          <StatGrid metrics={national} />
        </section>

        <section aria-label={t("pages.dataCenterPages.statesAria", { ns: "web_pages" })} className="space-y-4">
          <h2 className="text-2xl font-bold text-slate-900">{t("pages.dataCenterPages.statesTitle", { ns: "web_pages" })}</h2>
          {states.length === 0 ? (
            <p className="text-sm text-slate-500">{t("pages.dataCenterPages.statesRefreshing", { ns: "web_pages" })}</p>
          ) : (
        ...
        <section aria-label={t("pages.dataCenterPages.topMetrosAria", { ns: "web_pages" })} className="space-y-4">
          <h2 className="text-2xl font-bold text-slate-900">{t("pages.dataCenterPages.topMetrosTitle", { ns: "web_pages" })}</h2>
          {topMetros.length === 0 ? (
            <p className="text-sm text-slate-500">{t("pages.dataCenterPages.metrosRefreshing", { ns: "web_pages" })}</p>
          ) : (
