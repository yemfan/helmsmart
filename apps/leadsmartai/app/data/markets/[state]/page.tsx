...
function changeLine(
  ...
  const T = (k: string, v: Record<string, unknown> = {}) =>
    t(`pages.dataCenterPages.${k}`, { ns: "web_pages", ...v });
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 space-y-12">
        <nav className="text-sm">
          ...
          <Link href="/data" className="font-medium text-[#0072ce] hover:text-[#005ca8]">{t("pages.articleChrome.dataCenter", { ns: "web_pages" })}</Link>
          <span className="text-slate-400 mx-2">/</span>
          <Link href="/data/markets" className="font-medium text-[#0072ce] hover:text-[#005ca8]">{t("pages.dataCenterPages.markets", { ns: "web_pages" })}</Link>
          <span className="text-slate-400 mx-2">/</span>
          <span className="text-slate-600">{geo.geo_name}</span>
        </nav>

        <header className="space-y-4">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#0072ce]">{t("pages.dataCenterPages.stateTitle", { ns: "web_pages" })}</p>
          <h1 className="text-4xl font-bold leading-tight text-slate-900">
            {t("pages.dataCenterPages.stateHeroTitle", { ns: "web_pages", geo: geo.geo_name })}
          </h1>
          <p className="max-w-2xl text-lg leading-relaxed text-slate-600">
            {t("pages.dataCenterPages.stateHeroBody", { ns: "web_pages", geo: geo.geo_name })}
          ...
          {periodLabel && (
            <p className="text-xs text-slate-500">{t("pages.dataCenterPages.dataAsOf", { ns: "web_pages", period: periodLabel })}</p>
          )}
        </header>

        <section aria-label={t("pages.dataCenterPages.latestAria", { ns: "web_pages" })} className="space-y-4">
          <h2 className="text-2xl font-bold text-slate-900">{t("pages.dataCenterPages.latestTitle", { ns: "web_pages" })}</h2>
          <StatGrid metrics={metrics} />
        </section>

        {insightLines.length > 0 && (
          <section aria-label={t("pages.dataCenterPages.trendsAria", { ns: "web_pages" })} className="space-y-3">
            <h2 className="text-2xl font-bold text-slate-900">{t("pages.dataCenterPages.trendsTitle", { ns: "web_pages" })}</h2>
            <p className="max-w-2xl text-sm leading-relaxed text-slate-600">
              {t("pages.dataCenterPages.stateTrendsBody", { ns: "web_pages", geo: geo.geo_name })}
            </p>
        ...
        <section aria-label={t("pages.dataCenterPages.chartsAria", { ns: "web_pages" })} className="space-y-4">
          <h2 className="text-2xl font-bold text-slate-900">
            {t("pages.dataCenterPages.trendsHeading", { ns: "web_pages" })}
          </h2>
        ...
        <section aria-label={t("pages.dataCenterPages.metrosAria", { ns: "web_pages" })} className="space-y-4">
          <h2 className="text-2xl font-bold text-slate-900">
            {t("pages.dataCenterPages.metrosIn", { ns: "web_pages", geo: geo.geo_name })}
          </h2>
          <p className="max-w-2xl text-sm leading-relaxed text-slate-600">{t("pages.dataCenterPages.allLocal", { ns: "web_pages" })}</p>
          {metros.length === 0 ? (
            <p className="text-sm text-slate-500">
              {t("pages.dataCenterPages.noMetrosIn", { ns: "web_pages", geo: geo.geo_name })}
            </p>
          ) : (
            <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-left">
                  <tr>
                    <th className="px-4 py-2 font-semibold text-slate-700">{t("pages.dataCenterPages.metroArea", { ns: "web_pages" })}</th>
                    <th className="px-4 py-2 font-semibold text-slate-700">{t("pages.dataCenterPages.typicalValue", { ns: "web_pages" })}</th>
                  </tr>
                </thead>
                <tbody>
                  {metros.map((m) => (
