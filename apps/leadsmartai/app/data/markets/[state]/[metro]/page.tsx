    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 space-y-12">
        ...
        <header className="space-y-4">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#0072ce]">{t("pages.dataCenterPages.metroTitle", { ns: "web_pages" })}</p>
          <h1 className="text-4xl font-bold leading-tight text-slate-900">
            {t("pages.dataCenterPages.stateHeroTitle", { ns: "web_pages", geo: metroGeo.geo_name })}
          </h1>
          <p className="max-w-2xl text-lg leading-relaxed text-slate-600">
            {t("pages.dataCenterPages.metroHeroBody", { ns: "web_pages", geo: metroGeo.geo_name })}
            {isNum(vsState)
              ? ` ${t("pages.dataCenterPages.metroAnchor", {
                  ...
                  cmp:
                    t(
                      ...
                      { ns: "web_pages", abs: compareParts(vsState).abs ?? "", state: stName },
                    ) +
                    (isNum(vsNational)
                      ? t(
                          `pages.dataCenterPages.andNat${capitalize(compareParts(vsNational).kind)}`,
                          { ns: "web_pages", abs: compareParts(vsNational).abs ?? "" },
                        )
                      : ""),
                })}`
              : ""}
        ...
        {insightLines.length > 0 && (
          <section aria-label={t("pages.dataCenterPages.trendsAria", { ns: "web_pages" })} className="space-y-3">
            ...
            <p className="max-w-2xl text-sm leading-relaxed text-slate-600">
              {t("pages.dataCenterPages.metroTrendsBody", { ns: "web_pages", geo: metroGeo.geo_name })}
            </p>
        ...
        {otherInState.length > 0 && (
          <section aria-label={t("pages.dataCenterPages.otherMetrosAria", { ns: "web_pages" })} className="space-y-4">
            <h2 className="text-2xl font-bold text-slate-900">
              {t("pages.dataCenterPages.otherMetrosIn", { ns: "web_pages", geo: stName })}
            </h2>
