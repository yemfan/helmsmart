...
import { ensureAssistantsForAgent, updateAssistantForAgent } from "@/lib/realtorboss/assistants";
import { AI_TEAM, type AssistantType } from "@/lib/realtorboss/team";

export const runtime = "nodejs";

/**
 * POST /api/dashboard/realtorboss/assistant-avatar  (multipart/form-data)
...
export async function POST(req: Request) {
  try {
    ...
    console.error("POST /api/dashboard/realtorboss/assistant-avatar:", err);
...
export async function DELETE(req: Request) {
  try {
    ...
    const message = err instanceof Error ? err.message : "Server error";
    console.error("DELETE /api/dashboard/realtorboss/assistant-avatar:", err);
    return NextResponse.json({ ok: false, error: message }, { status: 500 });
  }
}
