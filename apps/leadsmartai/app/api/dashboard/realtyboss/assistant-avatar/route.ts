...
import { ensureAssistantsForAgent, updateAssistantForAgent } from "@/lib/realtyboss/assistants";
import { AI_TEAM, type AssistantType } from "@/lib/realtyboss/team";

export const runtime = "nodejs";

/**
 * POST /api/dashboard/realtyboss/assistant-avatar  (multipart/form-data)
...
export async function POST(req: Request) {
  try {
    ...
    console.error("POST /api/dashboard/realtyboss/assistant-avatar:", err);
...
export async function DELETE(req: Request) {
  try {
    ...
    const message = err instanceof Error ? err.message : "Server error";
    console.error("DELETE /api/dashboard/realtyboss/assistant-avatar:", err);
    return NextResponse.json({ ok: false, error: message }, { status: 500 });
  }
}
