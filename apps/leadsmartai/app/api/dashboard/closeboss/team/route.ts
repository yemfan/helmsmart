...
} from "@/lib/closeboss/assistants";
import { AI_TEAM, type AssistantType } from "@/lib/closeboss/team";
import { SKILLS } from "@/lib/ai/prompts/closeboss";
import { isValidAvatarId } from "@/lib/closeboss/avatars";

export const runtime = "nodejs";

/**
 * GET /api/dashboard/closeboss/team
...
export async function GET(req: Request) {
  try {
    ...
    console.error("GET /api/dashboard/closeboss/team:", err);
    return NextResponse.json({ ok: false, error: message }, { status: 500 });
  }
}

/**
 * PATCH /api/dashboard/closeboss/team
...
export async function PATCH(req: Request) {
  try {
    ...
    const message = err instanceof Error ? err.message : "Server error";
    console.error("PATCH /api/dashboard/closeboss/team:", err);
    return NextResponse.json({ ok: false, error: message }, { status: 500 });
  }
}
