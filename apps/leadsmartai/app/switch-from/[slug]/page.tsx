...
export async function generateMetadata({
  ...
  return {
    title,
    description: t(source.heroSubhead, { ns: "web_pages" }),
    keywords: [
    ...
    "@graph": [
      ...
      },
      {
        ...
        step: source.migrationSteps.map((key, i) => ({
          "@type": "HowToStep",
          position: i + 1,
          text: t(key, { ns: "web_pages" }).replace(/<SOURCE>/g, source.name),
        })),
      },
      {
        "@type": "FAQPage",
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950">
      ...
      <article className="mx-auto max-w-4xl px-4 py-12 md:px-6 md:py-16">
        <nav aria-label={t("pages.articleChrome.breadcrumb", { ns: "web_pages" })} className="mb-6 text-xs text-slate-500 dark:text-slate-400">
          ...
          <Link href="/switch-from" className="hover:text-slate-700 dark:hover:text-slate-200">{t("pages.switchFromPages.switchFromSlug", { ns: "web_pages" })}</Link>
          <span className="mx-2">/</span>
        ...
        {source.urgencyBanner ? (
          <div className="mb-6 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/40 dark:bg-amber-950/30">
            ...
            <p className="text-sm leading-6 text-amber-800 dark:text-amber-200">
              {t(source.urgencyBanner, { ns: "web_pages" })}
            </p>
          </div>
        ) : null}
        ...
        <header>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            >
              {t("pages.switchFrom.ctaTrial", { ns: "web_pages" })}
            </Link>
            <a
              href="#concierge"
            ...
            >
              {t("pages.switchFrom.wellMigrate", { ns: "web_pages" })}
            </a>
          </div>
          {source.companionPost ? (
            <p className="mt-5 text-sm text-slate-500 dark:text-slate-400">{t("pages.switchFromPages.relatedContext", { ns: "web_pages" })}{" "}
              <Link
                href={source.companionPost.href}
                className="font-semibold text-blue-700 hover:underline dark:text-blue-300"
              >
                {t(source.companionPost.label, { ns: "web_pages" })}
              </Link>
            </p>
          ) : null}
        </header>
        ...
        <section className="mt-12">
          <h2 className="text-xl font-semibold tracking-tight text-slate-900 md:text-2xl dark:text-white">{t("pages.switchFromPages.whyLeaving", { ns: "web_pages" })} {source.name}
          </h2>
            ...
            {source.painPoints.map((point) => (
              ...
              >
                <h3 className="text-base font-semibold text-slate-900 dark:text-white">
                  {t(point.title, { ns: "web_pages" })}
                </h3>
                <p className="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-300">
                  {t(point.body, { ns: "web_pages" })}
                </p>
              </li>
            ))}
          </ul>
        ...
        <section className="mt-12">
          <h2 className="text-xl font-semibold tracking-tight text-slate-900 md:text-2xl dark:text-white">{t("pages.switchFromPages.whereWeWin", { ns: "web_pages" })}</h2>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            {source.name}: {source.priceRange} · CloseBoss: $79 / mo starting · See the full table on the{" "}
            <Link
              href="/agent/compare"
              className="font-semibold text-blue-700 hover:underline dark:text-blue-300"
            >{t("pages.switchFromPages.comparisonPage", { ns: "web_pages" })}</Link>
            .
          </p>
          <div className="mt-5 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:bg-slate-900 dark:text-slate-400">
                <tr>
                  <th className="px-4 py-3">{t("pages.switchFromPages.feature", { ns: "web_pages" })}</th>
        ...
        <section className="mt-12">
          <h2 className="text-xl font-semibold tracking-tight text-slate-900 md:text-2xl dark:text-white">{t("pages.switchFromPages.migrateYourself", { ns: "web_pages" })}</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{t("pages.switchFromPages.singleSitting", { ns: "web_pages" })}{" "}
            <a
              href="#concierge"
              className="font-semibold text-blue-700 hover:underline dark:text-blue-300"
            >{t("pages.switchFromPages.skipToConcierge", { ns: "web_pages" })}</a>
            .
          </p>
            ...
            {source.migrationSteps.map((step, i) => (
              <li key={i} className="flex gap-4">
                ...
                <p className="flex-1 text-base leading-7 text-slate-700 dark:text-slate-200">
                  {t(step, { ns: "web_pages" }).replace(/<SOURCE>/g, source.name)}
                </p>
              </li>
            ))}
          </ol>
        ...
        >
          <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-blue-700 dark:text-blue-300">
            <span className="inline-flex items-center rounded-full bg-blue-600 px-2.5 py-1 text-white">
              <Sparkles className="mr-1 h-3 w-3" aria-hidden />{t("pages.switchFromPages.freeThrough", { ns: "web_pages" })}</span>
            <span>{t("pages.switchFromPages.conciergeMigration", { ns: "web_pages" })}</span>
          </div>
          <h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-900 md:text-3xl dark:text-white">{t("pages.switchFromPages.weMoveYourData", { ns: "web_pages" })}</h2>
          <p className="mt-3 max-w-2xl text-base leading-7 text-slate-700 dark:text-slate-200">{t("pages.switchFromPages.forAgentsFrom", { ns: "web_pages" })} {source.name}, we&apos;ll personally
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link
              ...
              className="inline-flex items-center justify-center rounded-md bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700"
            >{t("pages.switchFromPages.requestConcierge", { ns: "web_pages" })}<ArrowRight className="ml-2 h-4 w-4" aria-hidden />
            </Link>
            <Link
              href="/start-free"
              className="inline-flex items-center justify-center rounded-md border border-blue-200 bg-white px-5 py-2.5 text-sm font-semibold text-blue-700 transition hover:bg-blue-50 dark:border-blue-900/50 dark:bg-slate-900 dark:text-blue-300 dark:hover:bg-slate-900/70"
            >{t("pages.switchFromPages.orStartTrial", { ns: "web_pages" })}</Link>
          </div>
        </section>
        ...
        <section className="mt-12 border-t border-slate-200 pt-8 dark:border-slate-800">
          ...
          <div className="mt-6 space-y-6">
            {source.faq.map((entry, i) => (
              <div key={i}>
                <h3 className="text-base font-semibold text-slate-900 dark:text-white">
                  {t(entry.q, { ns: "web_pages" })}
                </h3>
                <p className="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-300">
                  {t(entry.a, { ns: "web_pages" })}
                </p>
              </div>
            ))}
          </div>
        ...
        <section className="mt-12 border-t border-slate-200 pt-8 text-center dark:border-slate-800">
          <p className="text-sm text-slate-500 dark:text-slate-400">{t("pages.switchFromPages.anotherCrm", { ns: "web_pages" })}{" "}
            <Link
              href="/switch-from"
              className="font-semibold text-blue-700 hover:underline dark:text-blue-300"
            >{t("pages.switchFromPages.seeAllGuides", { ns: "web_pages" })}</Link>
          </p>
        </section>
      </article>
    </div>
