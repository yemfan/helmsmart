  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <h1 className="text-3xl font-bold text-slate-900">
        {t("pages.seoCityPages.hvTitle", { ns: "web_pages", city: city.city, state: city.state })}
      </h1>
      <p className="mt-2 text-slate-700">
        {t("pages.seoCityPages.hvIntro", { ns: "web_pages", keyword: keywords[0], city: city.city })}
      </p>
      {asOf ? <p className="mt-1 text-xs text-slate-500">{t("pages.seoCityPages.dataAsOfDate", { ns: "web_pages", date: asOf })}</p> : null}
      ...
      {(market.typicalValue !== null ||
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {market.typicalValue !== null && (
            <Metric label={t("pages.seoCityPages.typicalHomeValue", { ns: "web_pages" })} value={formatCurrency(market.typicalValue)} />
          )}
          {market.yoyChangePct !== null && (
            <Metric
              label={t("pages.seoCityPages.oneYearChange", { ns: "web_pages" })}
              value={`${market.yoyChangePct > 0 ? "+" : ""}${market.yoyChangePct}%`}
            />
          )}
          {market.medianDaysOnMarket !== null && (
            <Metric label={t("pages.seoCityPages.medianDom", { ns: "web_pages" })} value={`${Math.round(market.medianDaysOnMarket)} days`} />
          )}
          {market.inventory !== null && (
            <Metric label={t("pages.seoCityPages.homesForSale", { ns: "web_pages" })} value={Math.round(market.inventory).toLocaleString(locale)} />
          )}
        </div>
      )}
      ...
      <section className="mt-8 grid gap-6 md:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="text-xl font-semibold text-slate-900">{t("pages.seoCityPages.sellerInsightIn", { ns: "web_pages", city: city.city })}</h2>
          <p className="mt-2 text-sm text-slate-700">
            {t("pages.seoCityPages.bestListingsBody", { ns: "web_pages", city: city.city })}
            {market.medianDaysOnMarket !== null
          ...
          <p className="mt-2 text-sm text-slate-700">
            {t("pages.seoCityPages.trendLine", { ns: "web_pages", city: city.city })} <span className="font-semibold">{market.trend}</span>
            {market.yoyChangePct !== null
              ? t("pages.seoCityPages.trendYoy", {
                  ns: "web_pages",
                  dir: t(market.yoyChangePct >= 0 ? "pages.seoCityPages.dirUp" : "pages.seoCityPages.dirDown", { ns: "web_pages" }),
                  pct: Math.abs(market.yoyChangePct),
                })
              : ""}
            .
          </p>
          <h3 className="mt-5 text-sm font-semibold uppercase tracking-wide text-slate-800">{t("pages.seoCityPages.keywordCoverage", { ns: "web_pages" })}</h3>
          ...
          <dl className="mt-2 space-y-4 text-sm text-slate-700">
            <div>
              <dt className="font-semibold text-slate-900">{t("pages.seoCityPages.howAccurateQ", { ns: "web_pages", keyword: keywords[0], city: city.city })}</dt>
              <dd className="mt-1 ml-0 text-slate-700">
                {t("pages.seoCityPages.strongestWith", { ns: "web_pages" })}
              </dd>
            </div>
            <div>
              <dt className="font-semibold text-slate-900">{t("pages.seoCityPages.goodTimeQ", { ns: "web_pages", city: city.city })}</dt>
              <dd className="mt-1 ml-0 text-slate-700">
                {market.trend === "up"
                  ? t("pages.seoCityPages.trendUpA", { ns: "web_pages" })
                  : market.trend === "down"
                    ? t("pages.seoCityPages.trendDownA", { ns: "web_pages" })
                    : t("pages.seoCityPages.trendFlatA", { ns: "web_pages" })}
              </dd>
            </div>
            <div>
              <dt className="font-semibold text-slate-900">{t("pages.seoCityPages.whatImpacts", { ns: "web_pages" })}</dt>
              <dd className="mt-1 ml-0 text-slate-700">
                {t("pages.seoCityPages.whatImpactsA", { ns: "web_pages" })}
              </dd>
            </div>
          </dl>
          <h3 className="mt-5 text-base font-semibold text-slate-900">{t("pages.seoCityPages.internalLinks", { ns: "web_pages" })}</h3>
          <div className="mt-2 flex flex-wrap gap-3 text-sm">
            {nearbyCities.map((near) => (
              <a key={near.slug} className="text-blue-700 hover:underline" href={`/home-value/${near.slug}`}>
                {t("pages.seoCityPages.nearbyHomeValues", { ns: "web_pages", city: near.city })}
              </a>
            ))}
          ...
          <p className="mt-5 rounded-lg bg-blue-50 px-4 py-3 text-sm text-blue-900">
            {t("pages.seoCityPages.readyForValuation", { ns: "web_pages" })}
          </p>
        </article>
        <LocalSeoLeadForm
          title={t("pages.seoCityPages.hvFormTitle", { ns: "web_pages", city: city.city })}
