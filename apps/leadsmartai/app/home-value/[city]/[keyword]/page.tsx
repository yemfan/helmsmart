...
export async function generateMetadata({
  ...
  return pageMetadata({
    ...
    path: `/home-value/${p.city}/${p.keyword}`,
  });
}
...
export default async function HomeValueKeywordPage({
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <p className="mt-2 text-slate-700">{t("pages.keywordPages.seoPageFor", { ns: "web_pages" })} {city.city}, {city.state}. Review current home value signals and build a
      ...
      {(market.typicalValue !== null ||
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          {market.typicalValue !== null && (
            <Metric label={t("pages.keywordPages.typicalValue", { ns: "web_pages" })} value={formatCurrency(market.typicalValue)} />
          )}
          {market.yoyChangePct !== null && (
            <Metric
              label={t("pages.keywordPages.oneYearTrend", { ns: "web_pages" })}
              value={`${market.yoyChangePct > 0 ? "+" : ""}${market.yoyChangePct}%`}
            />
          )}
          {market.medianDaysOnMarket !== null && (
            <Metric label={t("pages.keywordPages.daysOnMarket", { ns: "web_pages" })} value={`${Math.round(market.medianDaysOnMarket)}`} />
          )}
        </div>
      )}
      ...
      <section className="mt-8 grid gap-6 md:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="text-xl font-semibold text-slate-900">{t("pages.keywordPages.sellerInsight", { ns: "web_pages" })}</h2>
          <p className="mt-2 text-sm text-slate-700">{t("pages.keywordPages.the", { ns: "web_pages" })} {city.city} {t("pages.keywordPages.marketIsCurrently", { ns: "web_pages" })} <span className="font-semibold">{market.trend}</span>
            {market.medianDaysOnMarket !== null
              ? `, with a median market time of about ${Math.round(market.medianDaysOnMarket)} days`
              : ""}
            .
          </p>
          <h3 className="mt-4 text-base font-semibold text-slate-900">{t("pages.keywordPages.nearbyLinks", { ns: "web_pages" })}</h3>
          ...
          <h3 className="mt-4 text-base font-semibold text-slate-900">{t("pages.keywordPages.relatedPages", { ns: "web_pages" })}</h3>
          <div className="mt-2 flex flex-wrap gap-3 text-sm">
            <a href={`/sell-house/${city.slug}/${p.keyword}`} className="text-blue-700 hover:underline">{t("pages.keywordPages.sellHouseKeyword", { ns: "web_pages" })}</a>
            <a href={`/market-report/${city.slug}/${p.keyword}`} className="text-blue-700 hover:underline">{t("pages.keywordPages.marketReportKeyword", { ns: "web_pages" })}</a>
          </div>
        </article>
        <LocalSeoLeadForm title={t("pages.seoCityPages.hvFormTitle", { ns: "web_pages", city: city.city })} source="seo_home_value_keyword" city={city.city} />
      </section>
    </main>
  );
}
