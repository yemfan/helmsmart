  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <h1 className="text-3xl font-bold text-slate-900">
        {t("pages.seoCityPages.shTitle", { ns: "web_pages", city: city.city, state: city.state })}
      </h1>
      <p className="mt-2 text-slate-700">
        {t("pages.seoCityPages.shIntroFull", { ns: "web_pages", keyword: keywords[0], city: city.city })}
      </p>
      ...
      <section className="mt-8 grid gap-6 md:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="text-xl font-semibold text-slate-900">{t("pages.seoCityPages.localSellerInsights", { ns: "web_pages" })}</h2>
          <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-slate-700">
            {market.medianDaysOnMarket !== null && (
              <li>{t("pages.seoCityPages.timeOnMarketLi", { ns: "web_pages", days: Math.round(market.medianDaysOnMarket) })}</li>
            )}
            {market.yoyChangePct !== null && (
              <li>
                {t("pages.seoCityPages.yearChangeLi", { ns: "web_pages", pct: `${market.yoyChangePct > 0 ? "+" : ""}${market.yoyChangePct}` })}
              </li>
            )}
            {market.typicalValue !== null && (
              <li>{t("pages.seoCityPages.typicalValueLi", { ns: "web_pages", value: `$${Math.round(market.typicalValue).toLocaleString(locale)}` })}</li>
            )}
            {market.inventory !== null && (
              <li>{t("pages.seoCityPages.inventoryLi", { ns: "web_pages", count: Math.round(market.inventory).toLocaleString(locale) })}</li>
            )}
          </ul>
          <p className="mt-3 text-sm text-slate-700">
            {t("pages.seoCityPages.launchPricing", { ns: "web_pages" })}
          </p>
          ...
          <dl className="mt-2 space-y-4 text-sm text-slate-700">
            <div>
              <dt className="font-semibold text-slate-900">{t("pages.seoCityPages.howFastQ", { ns: "web_pages", city: city.city })}</dt>
              <dd className="mt-1 ml-0 text-slate-700">
                {t("pages.seoCityPages.medianTimelinesFull", { ns: "web_pages", dom: domText })}
              </dd>
            </div>
            <div>
              <dt className="font-semibold text-slate-900">{t("pages.seoCityPages.shouldIRenovate", { ns: "web_pages" })}</dt>
              <dd className="mt-1 ml-0 text-slate-700">
                {t("pages.seoCityPages.shouldIRenovateA", { ns: "web_pages" })}
              </dd>
            </div>
            <div>
              <dt className="font-semibold text-slate-900">{t("pages.seoCityPages.maximizeProceeds", { ns: "web_pages" })}</dt>
              <dd className="mt-1 ml-0 text-slate-700">
                {t("pages.seoCityPages.maximizeProceedsA", { ns: "web_pages" })}
              </dd>
            </div>
          </dl>
          ...
          <div className="mt-2 flex flex-wrap gap-3 text-sm">
            {nearbyCities.map((near) => (
              <a key={near.slug} className="text-blue-700 hover:underline" href={`/sell-house/${near.slug}`}>
                {t("pages.seoCityPages.sellNearby", { ns: "web_pages", city: near.city })}
              </a>
            ))}
          ...
          <p className="mt-5 rounded-lg bg-blue-50 px-4 py-3 text-sm text-blue-900">
            {t("pages.seoCityPages.sellCta", { ns: "web_pages" })}
          </p>
        </article>
        <LocalSeoLeadForm title={t("pages.seoCityPages.shFormTitle", { ns: "web_pages", city: city.city })} source="seo_sell_house_city" city={city.city} />
      </section>
    </main>
  );
}
