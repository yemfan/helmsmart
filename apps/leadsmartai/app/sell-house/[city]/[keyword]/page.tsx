...
export default async function SellHouseKeywordPage({
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <p className="mt-2 text-slate-700">{t("pages.keywordPages.sellerFocusedFor", { ns: "web_pages" })} {city.city}. Get timing, pricing, and demand insights built for this market.
      </p>

      <section className="mt-8 grid gap-6 md:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="text-xl font-semibold text-slate-900">{t("pages.keywordPages.marketSnapshot", { ns: "web_pages" })}</h2>
          <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-slate-700">
            {market.medianDaysOnMarket !== null && (
              <li>{t("pages.keywordPages.medianDaysOnMarket", { count: Math.round(market.medianDaysOnMarket), ns: "web_pages" })}</li>
            )}
            {market.yoyChangePct !== null && (
              <li>
                {t("pages.keywordPages.oneYearValueTrend", { ns: "web_pages" })} {market.yoyChangePct > 0 ? "+" : ""}
                {market.yoyChangePct}%
              </li>
            )}
            ...
            <li>{t("pages.keywordPages.currentDirection", { ns: "web_pages" })} {market.trend}</li>
          </ul>
        ...
        <LocalSeoLeadForm title={t("pages.seoCityPages.shFormTitle", { ns: "web_pages", city: city.city })} source="seo_sell_house_keyword" city={city.city} />
      </section>
    </main>
  );
}
