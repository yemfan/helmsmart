    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="mx-auto max-w-3xl px-4 py-12 space-y-10">
        <nav className="text-sm">
          ...
          <Link href="/newsletter" className="font-medium text-[#0072ce] hover:text-[#005ca8]">{t("pages.newsletterIssue.weeklyNewsletter", { ns: "web_pages" })}</Link>
          <span className="mx-2 text-slate-400">/</span>
        ...
        <header className="space-y-4">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#0072ce]">{t("pages.newsletterIssue.weekOf", { ns: "web_pages" })} {weekLabel} · {reg.name}
          </p>
        ...
        <section aria-label={t("pages.newsletterIssue.snapshotAria", { ns: "web_pages" })} className="space-y-4">
          <div className="flex items-baseline justify-between gap-3">
            <h2 className="text-2xl font-bold text-slate-900">
              {reg.name} {t("pages.newsletterIssue.marketSnapshot", { ns: "web_pages" })}</h2>
            <Link
              href={reg.dataHref}
              className="text-sm font-semibold text-[#0072ce] hover:text-[#005ca8]"
            >
              {t("pages.newsletter.fullData", { ns: "web_pages" })}
            </Link>
          </div>
          {reg.stats.length === 0 ? (
            <p className="rounded-2xl border border-dashed border-slate-300 bg-white p-6 text-center text-sm text-slate-500">{t("pages.newsletterIssue.dataFor", { ns: "web_pages" })} {reg.name} {t("pages.newsletterIssue.beingRefreshed", { ns: "web_pages" })}</p>
          ) : (
        ...
        <section aria-label={t("pages.newsletterIssue.thisWeekAria", { ns: "web_pages" })} className="space-y-6">
          <h2 className="text-2xl font-bold text-slate-900">{t("pages.newsletterIssue.thisWeek", { ns: "web_pages" })}</h2>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {it.why_it_matters && (
                        <p className="mt-2 text-sm leading-relaxed text-slate-600">
                          <span className="font-semibold text-slate-700">{t("pages.newsletterIssue.whatItMeans", { ns: "web_pages" })}{" "}
                          </span>
                          {it.why_it_matters}
                        </p>
                      )}
                      {it.source_url && (
                        <p className="mt-3 text-xs text-slate-500">
                          ...
                          >
                            {t("pages.newsletter.readSource", { ns: "web_pages" })}{it.publisher ? ` ${it.publisher}` : ""}
                          </a>
                        </p>
                      )}
                    </div>
        ...
        {/* Sources */}
        {sources.length > 0 && (
          <section aria-label={t("pages.articleChrome.sources", { ns: "web_pages" })} className="space-y-3">
            <h2 className="text-xl font-bold text-slate-900">{t("pages.articleChrome.sources", { ns: "web_pages" })}</h2>
        ...
        <footer className="border-t border-slate-200 pt-6 text-sm text-slate-500">{t("pages.newsletterIssue.provenance", { ns: "web_pages" })} {reg.name} {t("pages.newsletterIssue.provenanceTail", { ns: "web_pages" })}{" "}
          <Link href="/newsletter" className="font-medium text-[#0072ce] hover:underline">
            {t("pages.newsletter.subscribeOrBrowse", { ns: "web_pages" })}
          </Link>
        </footer>
      </div>
    </main>
