...
export default async function NewsletterHubPage() {
  ...
  return (
    <main className="bg-white text-slate-900">
      ...
      <section className="border-b border-slate-100 bg-gradient-to-b from-slate-50 to-white">
        <div className="mx-auto max-w-5xl px-6 py-16 md:py-20">
          <nav className="mb-6 text-sm">
            ...
            <span className="text-slate-600">{t("pages.newsletterIndex.weeklyNewsletter", { ns: "web_pages" })}</span>
          </nav>

          <div className="max-w-3xl">
            <p className="mb-4 inline-flex rounded-full border border-slate-200/90 bg-white px-3 py-1 text-xs font-medium uppercase tracking-[0.15em] text-slate-600 shadow-sm ring-1 ring-slate-900/[0.03]">{t("pages.newsletterIndex.weeklyNewsletter", { ns: "web_pages" })}</p>
            <h1 className="text-4xl font-bold leading-tight tracking-tight md:text-5xl">{t("pages.newsletterIndex.h1", { ns: "web_pages" })}</h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">{t("pages.newsletterIndex.sub", { ns: "web_pages" })}</p>
          </div>
      ...
      {/* Recent issues */}
      <section className="mx-auto max-w-5xl px-6 py-14 md:py-16" aria-label={t("pages.newsletterIndex.recentIssuesAria", { ns: "web_pages" })}>
        <div className="mb-6 flex items-baseline justify-between gap-4">
          <h2 className="text-2xl font-semibold tracking-tight md:text-3xl">{t("pages.newsletterIndex.recentIssues", { ns: "web_pages" })}</h2>
          <span className="text-sm text-slate-500">{t("pages.newsletterIndex.newestFirst", { ns: "web_pages" })}</span>
        </div>

        {digests.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center">
            <p className="text-slate-600">{t("pages.newsletterIndex.firstIssueComing", { ns: "web_pages" })}</p>
          </div>
        ) : (
              ...
              <li key={d.id}>
                ...
                >
                  <div className="flex flex-wrap items-center gap-3 text-xs">
                    <span className="rounded-full bg-[#0072ce]/10 px-2.5 py-1 font-semibold text-[#0072ce]">{t("pages.newsletterIndex.weekOf", { ns: "web_pages" })} {formatWeek(d.week_of, locale)}
                    </span>
                    <span className="text-slate-500">
                      {t("pages.newsletter.storyCount", { count: Array.isArray(d.items) ? d.items.length : 0, ns: "web_pages" })}
                    </span>
                  </div>
        ...
        {digests.length > 0 && regions.length > 1 && (
          <div className="mt-10 rounded-2xl border border-slate-200/80 bg-slate-50/70 p-6">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-500">{t("pages.newsletterIndex.readThisWeek", { ns: "web_pages" })}</h3>
            <p className="mt-1 text-sm text-slate-600">{t("pages.newsletterIndex.sameBriefing", { ns: "web_pages" })}</p>
