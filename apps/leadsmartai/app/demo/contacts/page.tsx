...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: t("pages.demoPages.metaContactsTitle", { ns: "web_pages" }),
    description: t("pages.demoPages.metaContactsDescription", { ns: "web_pages" }),
  ...
  return (
    <DemoShell active="/demo/contacts">
      <header className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-white">{t("pages.demoPages.contacts", { ns: "web_pages" })}</h1>
        ...
        <div className="flex flex-wrap gap-2">
          <DemoDisabledButton label={t("pages.demoPages.importCsv", { ns: "web_pages" })} variant="ghost" />
          <DemoDisabledButton label={t("pages.demoPages.newContact", { ns: "web_pages" })} />
        </div>
      </header>

      {/* Filter chips */}
      <div className="mb-4 flex flex-wrap gap-2">
        <Chip label={t("pages.demoPages.all", { ns: "web_pages" })} count={total} active />
        <Chip label={t("pages.demoPages.hotA", { ns: "web_pages" })} count={hot} />
        <Chip
          label={t("pages.demoPages.fromZillow", { ns: "web_pages" })}
          count={DEMO_CONTACTS.filter((c) => c.source === "Zillow").length}
        />
        <Chip
          label={t("pages.demoPages.openHouse", { ns: "web_pages" })}
          count={DEMO_CONTACTS.filter((c) => c.source === "Open House").length}
        />
        <Chip
          label={t("pages.demoPages.chipSpherePastClient", { ns: "web_pages" })}
          count={
            DEMO_CONTACTS.filter((c) => c.stage === "past_client").length
          }
        />
        <Chip
          label={t("pages.demoPages.escalated", { ns: "web_pages" })}
          count={escalated}
          tone="red"
        />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:bg-slate-900 dark:text-slate-400">
            <tr>
              <th className="px-4 py-3">{t("pages.demoPages.colName", { ns: "web_pages" })}</th>
              <th className="px-4 py-3">{t("pages.demoPages.colStage", { ns: "web_pages" })}</th>
              <th className="px-4 py-3">{t("pages.demoPages.colScore", { ns: "web_pages" })}</th>
              <th className="px-4 py-3">{t("pages.demoPages.colSource", { ns: "web_pages" })}</th>
              <th className="px-4 py-3">{t("pages.demoPages.colLastActivity", { ns: "web_pages" })}</th>
            </tr>
          </thead>
