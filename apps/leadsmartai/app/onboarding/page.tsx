    ...
    <>
      ...
      <noscript>
        <div className="mx-auto max-w-2xl px-4 py-16">
          <h1 className="text-3xl font-bold text-slate-900 mb-4">{t("pages.onboardingPage.getStarted", { ns: "web_pages" })}</h1>
          <p className="text-slate-700 leading-relaxed mb-4">{t("pages.onboardingPage.noJsBody", { ns: "web_pages" })}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Link
              href="/agent-signup"
              className="inline-flex rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-700"
            >{t("pages.onboardingPage.createAccount", { ns: "web_pages" })}</Link>
            <Link
              href="/pricing"
            ...
            >{t("pages.onboardingPage.seePricing", { ns: "web_pages" })}</Link>
          </div>
          <p className="mt-6 text-xs text-slate-500">{t("pages.onboardingPage.questions", { ns: "web_pages" })}{" "}
      ...
      <OnboardingFunnel
        fallback={
          <section className="mx-auto max-w-3xl px-4 py-16 text-center">
            <p className="mb-2 inline-flex rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium uppercase tracking-[0.15em] text-slate-600">{t("pages.onboardingPage.onboarding", { ns: "web_pages" })}</p>
            <h1 className="text-4xl font-bold tracking-tight text-slate-900 md:text-5xl">{t("pages.onboardingPage.h1", { ns: "web_pages" })}</h1>
            <p className="mt-5 text-lg leading-relaxed text-slate-600">{t("pages.onboardingPage.sub", { ns: "web_pages" })}</p>
            ...
            <p className="mt-6 text-xs text-slate-500" aria-live="polite">
              {t("pages.onboardingPage.loadingPreview", { ns: "web_pages" })}
            </p>
          </section>
        }
      />
