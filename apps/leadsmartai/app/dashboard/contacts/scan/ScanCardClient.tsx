  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{t("pages.scanCard.heading")}</h1>
      ...
      {step === "capture" && (
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-4">
          <div
            ...
            className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/60 px-6 py-12 transition hover:border-blue-400 hover:bg-blue-50/30"
          >
            ...
            <span className="mt-3 text-sm font-semibold text-slate-700 dark:text-slate-300">{t("pages.scanCard.tapToUpload")}</span>
      ...
      {/* Step 2: Processing */}
      {step === "processing" && (
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm text-center space-y-4">
          {preview && (
            <img src={preview} alt={t("pages.scanCard.cardAlt")} className="mx-auto max-h-48 rounded-lg border border-slate-200 dark:border-slate-700 object-contain" />
          )}
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-sm text-slate-600 dark:text-slate-400">{t("pages.scanCard.reading")}</span>
          </div>
        </div>
      )}
      ...
      {step === "review" && (
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm divide-y divide-slate-100 dark:divide-slate-800">
          {preview && (
            <div className="p-4">
              <img src={preview} alt={t("pages.scanCard.cardAlt")} className="mx-auto max-h-36 rounded-lg border border-slate-200 dark:border-slate-700 object-contain" />
            </div>
          )}
      ...
      {step === "saved" && (
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm text-center space-y-4">
          ...
          <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{t("pages.scanCard.saved")}</p>
