  ...
  return (
    <div className="inline-flex shrink-0 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-medium">
      <Link
        ...
        className={`px-3 py-1 transition ${ current === "list" ? "bg-[#0072ce] text-white" : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800" }`}
        aria-current={current === "list" ? "page" : undefined}
      >{t("pages.transactionsToggle.list")}</Link>
      <Link
        href="/dashboard/transactions/coordinator"
        className={`px-3 py-1 transition ${ current === "board" ? "bg-[#0072ce] text-white" : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800" }`}
