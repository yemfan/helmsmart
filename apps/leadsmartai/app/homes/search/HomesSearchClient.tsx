...
export default function HomesSearchClient({
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <label htmlFor="homes-q" className="text-sm font-semibold text-slate-900">{t("pages.homesSearch.describeIdeal", { ns: "web_pages" })}</label>
        <textarea
          id="homes-q"
          value={query}
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          <span className="text-xs text-slate-400">
            {usage?.reached
              ? usage.reason === "free_limit"
                ? t("pages.homesSearch.youVeHitToday", { ns: "web_pages" })
                : t("pages.homesSearch.youVeUsedToday", { ns: "web_pages" })
              : t("pages.homesSearch.searchesTheLiveWeb", { ns: "web_pages" })}
          </span>
          <button
            type="submit"
          ...
          >
            {loading ? t("common:status.searching") : t("pages.homesSearch.searchHomes", { ns: "web_pages" })}
          </button>
        </div>
        {usage && usage.tier === "anonymous" && !usage.reached ? (
          <p className="mt-2 text-[11px] text-slate-400">{t("pages.homesSearch.oneFreePerDay", { ns: "web_pages" })}</p>
        ) : null}
        {usage && usage.tier === "free" && !usage.reached && usage.remaining != null ? (
          <p className="mt-2 text-[11px] text-slate-400">
            {usage.remaining} of {usage.limit} {t("pages.homesSearch.freeLeft", { ns: "web_pages" })}</p>
        ) : null}
      </form>
      ...
      {rateLimited?.reason === "anon_limit" ? (
        <div className="rounded-2xl border border-blue-200 bg-blue-50 px-5 py-5 text-sm text-slate-800">
          <p className="text-base font-semibold text-slate-900">{t("pages.homesSearch.usedTodays", { ns: "web_pages" })}</p>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            >{t("pages.homesSearch.createAccount", { ns: "web_pages" })}</Link>
            <Link href={SIGNIN_HREF} className="text-sm font-semibold text-blue-700 hover:underline">{t("pages.homesSearch.signIn", { ns: "web_pages" })}</Link>
          </div>
          <p className="mt-3 text-xs text-slate-500">{t("pages.homesSearch.comeBackTomorrow", { ns: "web_pages" })}</p>
        </div>
      ) : null}

      {rateLimited?.reason === "free_limit" ? (
        <div className="rounded-2xl border border-blue-200 bg-blue-50 px-5 py-5 text-sm text-slate-800">
          <p className="text-base font-semibold text-slate-900">{t("pages.homesSearch.hitLimit", { ns: "web_pages" })}</p>
          <p className="mt-1 text-slate-700">{rateLimited.message}</p>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <Link
              href={UPGRADE_HREF}
              className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700"
            >{t("pages.homesSearch.upgrade", { ns: "web_pages" })}</Link>
          </div>
          <p className="mt-3 text-xs text-slate-500">{t("pages.homesSearch.comeBackTomorrowPl", { ns: "web_pages" })}</p>
        </div>
      ) : null}
      ...
      {result && !loading ? (
        <>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-sm font-semibold text-slate-900">{t("pages.homesSearch.howWeRead", { ns: "web_pages" })}</h2>
            ...
            {result.refinements.length > 0 ? (
              <div className="mt-4 border-t border-slate-100 pt-4">
                <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">{t("pages.homesSearch.narrowSearch", { ns: "web_pages" })}</h3>
          ...
          {result.listings.length === 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-600">{t("pages.homesSearch.noMatches", { ns: "web_pages" })}</div>
          ) : (
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex sm:items-center sm:justify-between">
            <div>
              <div className="text-sm font-semibold text-slate-900">{t("pages.homesSearch.wantHelp", { ns: "web_pages" })}</div>
              <p className="mt-1 text-xs text-slate-600">{t("pages.homesSearch.connectBody", { ns: "web_pages" })}</p>
            </div>
            <button
              type="button"
            ...
            >{t("pages.homesSearch.connectAgent", { ns: "web_pages" })}</button>
          </section>

          {/* Sources + disclaimer */}
          {result.sources.length > 0 ? (
            <div className="rounded-xl border border-slate-200 bg-white px-4 py-3">
              <h3 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{t("pages.articleChrome.sources", { ns: "web_pages" })}</h3>
    ...
    <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3 border-t border-slate-100 pt-3">
        {listing.listingUrl ? (
          ...
          >
            {t("pages.homesSearch.viewListing", { ns: "web_pages" })}
          </a>
        ) : null}
        <button
          type="button"
        ...
        >{t("pages.homesSearch.askAnAgent", { ns: "web_pages" })}</button>
      </div>
    </div>
  );
}
