  ...
  return (
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="bg-white shadow rounded-xl p-6 border border-gray-100">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">{t("pages.smartCmaBuilder.h1", { ns: "web_pages" })}</h1>
        <p className="text-sm text-gray-600 mb-4">{t("pages.smartCmaBuilder.sub", { ns: "web_pages" })}</p>
        {usage ? (
          <div className="mb-4 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            <div className="text-xs text-slate-700 font-medium">{t("pages.smartCmaBuilder.youHaveUsed", { ns: "web_pages" })} {usage.used}/{usage.limit} {t("pages.smartCmaBuilder.reportsToday", { ns: "web_pages" })}</div>
            {usage.warning && !usage.reached ? (
              <div className="text-[11px] text-amber-700 mt-1">
                {t("pages.smartCmaBuilder.almostOutOfFreeReports", { ns: "web_pages" })}
              </div>
            ) : null}
            {usage.reached ? (
              <div className="text-[11px] text-red-700 mt-1">{t("pages.smartCmaBuilder.limitReached", { ns: "web_pages" })}</div>
            ) : null}
          </div>
        ) : null}
        <div className="space-y-3">
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div>
                <div className="text-xs font-semibold text-slate-700">{t("pages.smartCmaBuilder.attachToLead", { ns: "web_pages" })}</div>
                <div className="text-[11px] text-slate-500 mt-1">{t("pages.smartCmaBuilder.attachNote", { ns: "web_pages" })}</div>
              </div>
              <div className="flex-1 sm:max-w-[420px]">
                {leadOptionsLoading ? (
                  <div className="text-xs text-slate-600">{t("pages.smartCmaBuilder.loadingLeads", { ns: "web_pages" })}</div>
                ) : null}
                {leadOptionsError ? (
                ...
                >
                  <option value="">{t("pages.smartCmaBuilder.doNotAttach", { ns: "web_pages" })}</option>
                  {leadOptions.map((l) => (
            ...
            {attachedLead ? (
              <div className="text-xs text-slate-600">{t("pages.smartCmaBuilder.attachedTo", { ns: "web_pages" })}{" "}
                <span className="font-semibold">{attachedLead.name ?? t("pages.notifications.lead")}</span> •{" "}
                {attachedLead.property_address ?? t("pages.smartCmaBuilder.noAddress", { ns: "web_pages" })}
              </div>
            ) : (
              <div className="text-xs text-slate-600">{t("pages.smartCmaBuilder.noLeadAttached", { ns: "web_pages" })}</div>
            )}

            {attachedLeadId && attachedLead?.property_address ? (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div className="text-[11px] text-slate-500">{t("pages.smartCmaBuilder.useLeadAddress", { ns: "web_pages" })}</div>
                <button
                  type="button"
                ...
                >{t("pages.smartCmaBuilder.createAndSave", { ns: "web_pages" })}</button>
              </div>
            ) : null}
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-3 items-stretch">
            <input
              ...
              placeholder={t("pages.smartCmaBuilder.propertyAddress", { ns: "web_pages" })}
            ...
            >
              ...
              {loading ? t("common:status.generating") : t("pages.smartCmaBuilder.generateCma", { ns: "web_pages" })}
            </button>
            <button
            ...
            >{t("pages.smartCmaBuilder.refreshData", { ns: "web_pages" })}</button>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
            <LabeledInput
              label={t("pages.articleChrome.beds", { ns: "web_pages" })}
              value={beds ?? ""}
              onChange={(v) => setBeds(v ? Number(v) || 0 : undefined)}
            />
            <LabeledInput
              label={t("pages.articleChrome.baths", { ns: "web_pages" })}
              value={baths ?? ""}
              onChange={(v) => setBaths(v ? Number(v) || 0 : undefined)}
            />
            <LabeledInput
              label={t("pages.articleChrome.sqft", { ns: "web_pages" })}
              value={sqft ?? ""}
              onChange={(v) => setSqft(v ? Number(v) || 0 : undefined)}
            />
            <LabeledInput
              label={t("pages.articleChrome.yearBuilt", { ns: "web_pages" })}
              value={yearBuilt ?? ""}
              onChange={(v) =>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            <label className="flex flex-col">
              <span className="font-semibold text-gray-600 mb-1">{t("pages.smartCmaBuilder.condition", { ns: "web_pages" })}</span>
              <select
                value={condition}
                onChange={(e) => setCondition(e.target.value)}
                className="border border-gray-300 rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option>{t("pages.smartCmaBuilder.needsWork", { ns: "web_pages" })}</option>
                <option>{t("pages.smartCmaBuilder.average", { ns: "web_pages" })}</option>
                <option>{t("pages.smartCmaBuilder.updated", { ns: "web_pages" })}</option>
                <option>{t("pages.smartCmaBuilder.fullyRenovated", { ns: "web_pages" })}</option>
              </select>
            </label>
          </div>
        </div>
      ...
      {data && (
        <>
          <div className="bg-white shadow rounded-xl p-6 border border-gray-100">
            <h2 className="text-lg font-semibold text-gray-900 mb-2">{t("pages.smartCmaBuilder.propertySummary", { ns: "web_pages" })}</h2>
            ...
            <p className="text-xs text-gray-600">{t("pages.smartCmaBuilder.built", { ns: "web_pages" })} {data.subject.yearBuilt} • Condition:{" "}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white shadow rounded-lg p-4 border border-gray-100">
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">{t("pages.smartCmaBuilder.estimatedValue", { ns: "web_pages" })}</h3>
            ...
            <div className="bg-white shadow rounded-lg p-4 border border-gray-100">
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">{t("pages.smartCmaBuilder.priceRange", { ns: "web_pages" })}</h3>
            ...
            <div className="bg-white shadow rounded-lg p-4 border border-gray-100">
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">{t("pages.smartCmaBuilder.strategySnapshot", { ns: "web_pages" })}</h3>
              <p className="text-xs text-gray-700">{t("pages.smartCmaBuilder.aggressive", { ns: "web_pages" })}{" "}
                <span className="font-semibold">
                  {formatCurrency(data.strategies.aggressive)}
                </span>{" "}
                {t("pages.smartCmaBuilder.approxDays", { ns: "web_pages", count: data.strategies.daysOnMarket.aggressive })}
              </p>
              <p className="text-xs text-gray-700">{t("pages.smartCmaBuilder.market", { ns: "web_pages" })}{" "}
                <span className="font-semibold">
                  {formatCurrency(data.strategies.market)}
                </span>{" "}
                {t("pages.smartCmaBuilder.approxDays", { ns: "web_pages", count: data.strategies.daysOnMarket.market })}
              </p>
              <p className="text-xs text-gray-700">{t("pages.smartCmaBuilder.premium", { ns: "web_pages" })}{" "}
                <span className="font-semibold">
                  {formatCurrency(data.strategies.premium)}
                </span>{" "}
                {t("pages.smartCmaBuilder.approxDays", { ns: "web_pages", count: data.strategies.daysOnMarket.premium })}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white shadow rounded-xl p-6 border border-gray-100">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-semibold text-gray-900">{t("pages.articleChrome.comparableSales", { ns: "web_pages" })}</h2>
              ...
              <table className="min-w-full text-xs border-collapse">
                <thead>
                  <tr className="bg-gray-50 text-left text-gray-600">
                    <th className="px-3 py-2 font-semibold">{t("pages.articleChrome.address", { ns: "web_pages" })}</th>
                    <th className="px-3 py-2 font-semibold">{t("pages.smartCmaBuilder.soldPrice", { ns: "web_pages" })}</th>
                    <th className="px-3 py-2 font-semibold">{t("pages.articleChrome.sqft", { ns: "web_pages" })}</th>
                    <th className="px-3 py-2 font-semibold">{t("pages.smartCmaBuilder.pricePerSqft", { ns: "web_pages" })}</th>
                    <th className="px-3 py-2 font-semibold">{t("pages.articleChrome.beds", { ns: "web_pages" })}</th>
                    <th className="px-3 py-2 font-semibold">{t("pages.articleChrome.baths", { ns: "web_pages" })}</th>
                    <th className="px-3 py-2 font-semibold">{t("pages.smartCmaBuilder.distance", { ns: "web_pages" })}</th>
                    <th className="px-3 py-2 font-semibold">{t("pages.smartCmaBuilder.soldDate", { ns: "web_pages" })}</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-6 text-sm text-blue-900">
            <h2 className="text-sm font-semibold mb-2">{t("pages.smartCmaBuilder.aiSummary", { ns: "web_pages" })}</h2>
            <p>{data.summary}</p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >{t("pages.smartCmaBuilder.downloadPdf", { ns: "web_pages" })}</button>
            ...
            >
              {savingReport ? t("common:status.saving") : t("pages.smartCmaBuilder.saveToReports", { ns: "web_pages" })}
            </button>
          </div>
          ...
          {savedReportId ? (
            <div className="pt-2">
              ...
              >{t("pages.smartCmaBuilder.openSaved", { ns: "web_pages" })}</Link>
            </div>
          ) : null}
    ...
    <PaywallModal
      ...
      message={t("pages.smartCmaBuilder.paywallMessage", { ns: "web_pages" })}
      ctaLabel={t("pages.smartCmaBuilder.paywallCta", { ns: "web_pages" })}
    />
    {paywallOpen ? (
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50">
        ...
        >{t("pages.smartCmaBuilder.tryTomorrow", { ns: "web_pages" })}</button>
      </div>
    ) : null}
    </>
  );
