 ...
 *
 * Reads  public/brand/closeboss/closeboss-mark.svg
 * Writes public/brand/closeboss/closeboss-icon-{64,180,512}.png
 *        (mark centered on a white rounded-rect tile, like the
 *        approved app-icon concept) and closeboss-mark-512.png
 *        (transparent, no tile).
 *
 * Usage (from apps/leadsmartai): node ./scripts/generate-closeboss-icons.mjs
 */
...
const brandDir = join(__dirname, "..", "public", "brand", "closeboss");
mkdirSync(brandDir, { recursive: true });

const markSvg = readFileSync(join(brandDir, "closeboss-mark.svg"), "utf8");
...
for (const size of [64, 180, 512]) {
  const png = await sharp(Buffer.from(tileSvg(size)), { density: 300 })
    ...
    .png()
    .toBuffer();
  writeFileSync(join(brandDir, `closeboss-icon-${size}.png`), png);
  console.log(`closeboss-icon-${size}.png`);
}
...
const mark = await sharp(Buffer.from(markSvg), { density: 300 })
  ...
  .png()
  .toBuffer();
writeFileSync(join(brandDir, "closeboss-mark-512.png"), mark);
console.log("closeboss-mark-512.png");
