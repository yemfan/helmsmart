...
export const createCrmTask = defineTool({
  ...
  inputSchema: z.object({
    ...
    due_at: z.string().datetime({ offset: true, local: true }).optional().describe("ISO timestamp"),
  }),
  riskClass: "crm_write",
...
export const createCalendarEvent = defineTool({
  ...
  inputSchema: z.object({
    title: z.string().min(3).max(200),
    description: z.string().max(2000).optional(),
    starts_at: z.string().datetime({ offset: true, local: true }).describe("ISO start time"),
    ends_at: z.string().datetime({ offset: true, local: true }).optional().describe("ISO end time"),
    contact_id: z.string().uuid().optional(),
  }),
  riskClass: "crm_write",
