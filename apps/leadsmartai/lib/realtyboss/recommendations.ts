...
import { pausedAssistantTypes } from "@/lib/realtyboss/assistants";
...
export async function syncBossRecommendations(agentId: string): Promise<void> {
  ...
  if (candidates.length > 0) {
    ...
    if (error) console.warn("[realtyboss] recommendation upsert failed:", error.message);
  }
  ...
  if (openErr) {
    console.warn("[realtyboss] recommendation stale-check failed:", openErr.message);
    return;
  }
