...
export function buildSkillsPackMarkdown(): string {
  ...
  lines.push(
    `${count} ready-to-run AI skills for real estate agents — each with the value it captures and a compliance-baked prompt. ` +
      "Free, from **RealtyBoss** (realtybossai.com). Wire the placeholders to your own agent + client data and drop each prompt into your AI of choice.",
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "RealtyBoss gives you an AI real estate team that runs this whole library — assigned to a Receptionist, Sales, Marketing, Transaction, and Accounting assistant, with a Boss Assistant that runs the compliance gate on every output. Just ask, and the right assistant does it. Start free at **realtybossai.com**.",
  );
  lines.push("");
