...
/**
 * AI House Search producer — Claude + live web search turns a buyer's
 * natural-language brief into real, currently-listed homes with cited
 * source URLs, plus suggested refinements. Same Claude + web_search +
 * pause_turn pattern as lib/cma/aiCma.ts.
 *
 * IMPORTANT — Claude is instructed to only report listings it actually
 * finds via search (with a source URL each), never to invent an address,
 * price, or listing. Sparse real results beat fabricated ones.
 */
