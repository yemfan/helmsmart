...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!geo) return { title: "State housing market | PropertyTools AI Data Center" };

  const title = `${geo.geo_name} Housing Market Data — Prices, Inventory & Trends | PropertyTools AI`;
  const description = `Current ${geo.geo_name} housing-market data: typical home value, median sale price, inventory, and days on market — with month-over-month and year-over-year trends and how ${geo.geo_name} compares to the U.S.`;
  return {
    title,
    description,
...
export default async function StatePage({ params }: Props) {
  ...
  const datasetJsonLd = {
    ...
    creator: { "@type": "Organization", name: "PropertyTools AI" },
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 space-y-12">
        <nav className="text-sm">
          ...
          <span className="text-slate-400 mx-2">/</span>
          <Link href="/data/markets" className="font-medium text-blue-700 hover:underline">
            Markets
          </Link>
          <span className="text-slate-400 mx-2">/</span>
        ...
        <header className="space-y-4">
          ...
          <h1 className="text-4xl font-bold leading-tight text-slate-900">
            {geo.geo_name} housing market
          </h1>
          <p className="max-w-2xl text-lg leading-relaxed text-slate-600">
            The latest {geo.geo_name} home prices, inventory, and days-on-market —
            with month-over-month and year-over-year trends
            {isNum(vsNational) && isNum(zhvi?.value) ? (
              <>
                {" "}and how it stacks up nationally. The typical {geo.geo_name} home
                is <strong>{comparePhrase(vsNational)}</strong> the U.S. typical value
                of {formatValue(natZhvi?.value ?? null, natZhvi?.unit ?? "index", { compact: true })}.
              </>
            ) : (
              "."
            )}
        ...
        <section aria-label="Latest metrics" className="space-y-4">
          <h2 className="text-2xl font-bold text-slate-900">Latest market snapshot</h2>
          <StatGrid metrics={metrics} />
        </section>

        {insightLines.length > 0 && (
          <section aria-label="Trends and insights" className="space-y-3">
            <h2 className="text-2xl font-bold text-slate-900">What the numbers say</h2>
        ...
        <section aria-label="Trend charts" className="space-y-4">
          <h2 className="text-2xl font-bold text-slate-900">13-month trends</h2>
