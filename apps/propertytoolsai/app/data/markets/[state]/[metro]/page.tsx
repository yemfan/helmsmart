...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!resolved) return { title: "Metro housing market | PropertyTools AI Data Center" };

  const { metro: m } = resolved;
  const title = `${m.geo_name} Housing Market Data — Prices, Inventory & Trends | PropertyTools AI`;
  const description = `Current ${m.geo_name} housing-market data: typical home value, median sale price, inventory, and days on market — with month-over-month and year-over-year trends and how ${m.geo_name} compares to its state and the U.S.`;
  return {
    title,
    description,
    alternates: {
