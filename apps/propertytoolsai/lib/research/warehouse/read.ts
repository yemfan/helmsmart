 ...
 * and match on the computed slug. Results are memoized per request via
 * `cache()` so a page that both resolves a slug and lists geos hits the DB once.
 */
 ...
 * period for that geography (cheap: one grouped query for the latest period per
 * geo). Never throws — returns [] on any failure so the sitemap can't crash.
 */
